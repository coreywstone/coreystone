...
function SketchesRow() {
  const sketchesSections = [
    { 
      id: 'intro', 
      label: 'INTRO',
      content: (
        <div className="textbox-image-pair">
          <div className="textbox-image-pair-textbox" style={{ width: '100%', maxWidth: '600px', padding: '24px', boxSizing: 'border-box' }}>
  ...
  const [activeSectionId, setActiveSectionId] = useState(sketchesSections[0]?.id || null)
  ...
  // Set up IntersectionObserver to detect when SketchesRow is in view (for sticky nav)
  useEffect(() => {
  ...
  useEffect(() => {
    if (sketchesSections.length === 0 || !scrollContainerRef.current) return
    ...
    <section ref={rowRef} className="sketches-murals-row">
      ...
      <div ref={scrollContainerRef} className="sketches-murals-panel-container">
        <div className="sketches-murals-panel-scroll">
          {sketchesSections.length > 0 ? (
            sketchesSections.map((section, index) => (
              <SketchesMuralsSection
                key={section.id}
                ref={el => sectionRefs.current[index] = el}
                id={section.id}
                isLast={index === sketchesSections.length - 1}
