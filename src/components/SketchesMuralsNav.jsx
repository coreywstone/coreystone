    ...
    <nav ref={navRef} className={`sketches-murals-nav ${isSticky ? 'sticky' : ''}`}>
      ...
      {showTabs && sections.length > 0 && (
        <div ref={tabsContainerRef} className="sketches-murals-nav-tabs">
          <div 
            ref={indicatorRef}
            className={`sketches-murals-nav-indicator ${isAnimating ? 'animating' : ''}`}
          ...
          {sections.map((section, index) => (
            <button
              ...
              className={`sketches-murals-nav-tab ${activeSectionId === section.id ? 'active' : ''}`}
...
export default SketchesMuralsNav
